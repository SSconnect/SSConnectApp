...
const inPageInfo = (state, props) =>
  inProfilePage(state, props).get("pageInfo")

const selectConfig = createSelector(selectGlobal, state => state.get("config"))
const selectPremium = createSelector(selectGlobal, state =>
  state.get("premium")
)
const selectLoading = createSelector(selectGlobal, state =>
  state.get("loading")
)
...
const makeSelectPageInfo = () =>
  createSelector([inPageInfo], state => state || { page: 1 })
const makeSelectStories = () =>
  createSelector([inStories], state => state || [])
